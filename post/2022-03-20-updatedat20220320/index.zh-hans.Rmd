...
actbase <- (MC10KAct |> filter(year(Edate)==2010))$FLOW
# MTLF预测值New
...
cbnFctAct |>
    # filter(MdlName=="MTLF") |> 
    filter(MdlName %in% c("GenArima","ArimaMA2","GenEts","EtsMA2","GenPp","PpMA2")) |>
    ...
    mutate(Yr=Y-nFctYr) |> 
    mutate(Fct_MN=Fct_MN/actbase,FLOW=FLOW/actbase) |> #归一化，保密数据
